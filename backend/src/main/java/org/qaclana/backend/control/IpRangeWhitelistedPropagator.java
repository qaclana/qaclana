...
/**
 * Propagates the information about the whitelisting of a specific {@link IpRange}.
 *
 * @author Juraci Paixão Kröhling
 */
@Stateless
public class IpRangeWhitelistedPropagator {
    @Inject
    @Firewall
     ...
     *
     * @param ipRangeAddedToWhitelist the new whitelisted {@link IpRange}
     */
    @Asynchronous
    public void propagate(@Observes IpRangeAddedToWhitelist ipRangeAddedToWhitelist) {
        BasicMessage message = new IpRangeAddedToWhitelistMessage(ipRangeAddedToWhitelist.getIpRange());
     ...
     * {@link ManagedExecutorService} to execute the submissions.
     *
     * @param ipRangeRemovedFromWhitelist the {@link IpRange} that has been removed from the whitelist
     */
    @Asynchronous
    public void propagate(@Observes IpRangeRemovedFromWhitelist ipRangeRemovedFromWhitelist) {
        BasicMessage message = new IpRangeRemovedFromWhitelistMessage(ipRangeRemovedFromWhitelist.getIpRange());
        firewallSessionsInstance.get().forEach((sessionId, session) ->
                executor.submit(
                        () -> sendMessageEvent.fire(new SendMessage(session, message))
                )
    ...
    /**
     * Sends the whitelist to new firewall instances
     *
    ...
    public void propagate(@Observes NewFirewallInstanceRegistered newFirewallInstanceRegistered) {
        whitelistService.list().stream().forEach(ipRange -> {
            BasicMessage message = new IpRangeAddedToWhitelistMessage(ipRange);
            executor.submit(
